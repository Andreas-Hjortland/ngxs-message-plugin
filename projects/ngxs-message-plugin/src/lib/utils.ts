 ...
 */
function mergeList<T>(objA: T[], objB: T[], seen: Set<any>): MergeResult<T[]> {
  let i = 0;
  let result: any = [];
  let equal = true;
  for (; i < objA.length && i < objB.length; i++) {
    const merged = deepMerge(objA[i], objB[i], seen);
 ...
 */
function mergeObject<T>(objA: T, objB: T, seen: Set<any>): MergeResult<T> {
  const result = {} as T;
  let equal = true;
  ...
  for (const key in objA) {
    if (Object.prototype.hasOwnProperty.call(objA, key)) {
      ...
      if (Object.prototype.hasOwnProperty.call(objB, key)) {
        let merged = deepMerge(objA[key], objB[key], seen);
...
export function deepMerge<T>(
  ...
  if (typeof objA === 'object' && objA !== null) {
    if (seen.has(objA)) {
      console.error('Found cycle in object tree. Cycle detected in', objA);
      throw new Error('Object cycle detected');
    }
    seen.add(objA);
  }
  ...
  if (objA === objB) {
    ...
    return mergeList(objA, objB, seen) as unknown as MergeResult<T>;
  } else if (
    objA &&
    objB &&
    typeof objA === 'object' &&
    ...
    // object that arent `null`
    return mergeObject(objA, objB, seen);
  } else {
