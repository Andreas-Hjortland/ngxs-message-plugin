...
import { ACTION_DISPATCHED, GET_STORE, KNOWN_ACTIONS, MessageCommunicationService, STORE_UPDATE } from './symbols';
...
export class HostHandler implements OnDestroy {
    ...
    constructor(
        ...
        this.knownActions = new Map(knownActions?.flat().map(actionConstructor => [actionConstructor.type, actionConstructor]));
    }
    ...
    private onActionDispatched(actionType: string | undefined, action: any): void {
        if (actionType) {
            const actionConstructor = this.knownActions.get(actionType);
            const constructedAction = actionConstructor ? Object.create(actionConstructor.prototype) : new class MessageAction {
