...
export type Config = {
     ...
     *   instances of the app running with `forRoot`
     * - `custom` will not register any implementations of 
     *   `MessageCommunicationService` so you will have to ship your own service
     */
    messageHandler?: 'custom' | 'broadcast' | 'port';
 ...
 */
export const BROADCAST_CHANNEL_NAME = new InjectionToken<string>('BROADCAST_CHANNEL_NAME');
 ...
 * communication type
 */
export const ALLOWED_ORIGIN = new InjectionToken<string | string[]>('ALLOWED_ORIGIN');

/**
 * Known actions. Used to map prototypes to objects after deserialization
 */
export const KNOWN_ACTIONS = new InjectionToken<ActionDef[] | ActionDef>('KNOWN_ACTION');
